...
Use the [repository-local FI-0 prompt](docs/ppphp-framework-integration-fi0-codex-prompt.md) for the framework evidence spike. It contains the full work order and supersedes the attachment-only FI-0 and alternate Stage 17A handoffs. No chat attachment or conversation history is required. Before handing work to another agent, commit every required input or embed its requirements in the prompt, verify repository paths and links, and distinguish pre-existing inputs from outputs to create. Preserve existing stage identifiers and report PASS, FAIL and NOT RUN honestly. Local feature branches may isolate work, but consolidate to local `develop` before pushing; push only `develop`, never a remote feature branch, and never force-push to bypass concurrent work.
