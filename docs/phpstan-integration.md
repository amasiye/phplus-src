...
`PhpStanProjectAnalyzer` is instantiated lazily after compiler-owned success. Thus browser protocol version 2 can use the same compiler semantics, including an explicitly mounted portable dependency index, without constructing PHPStan, while ordinary `check` and `build` retain the existing full guarantees. An exact successful supplemental result may be reused only when compiler, project, workspace-input, host-PHP, PHPStan executable/package/configuration, and process-policy identities all match. Process failures are never cached. Catalog version 4 has no required compiler gaps and reports `fullParity: true`; the compiler-only result is still not exposed as a public CLI/configuration mode.
...
On a supplemental-cache miss, each check prepares `.ppphp-cache/analysis/` with deterministic areas for selected files, unselected context, configured stubs, backend configuration, mappings, results, and temporary backend data. A source-root hash prevents equal relative paths from different roots from colliding. An exact cache hit skips workspace preparation and process launch while replaying the same validated stable diagnostics.
...
`PhpStanProjectAnalyzer` implements the compiler-owned `ProjectAnalyzer` interface. It resolves the Composer-installed PHPStan binary under the compiler package and invokes it through `PHP_BINARY` and a shared hardened process runner with argument-array execution, no shell, JSON output, no progress display, bounded standard output/error, a finite timeout, termination and reaping, and a generated configuration. Only a reviewed environment allowlist is forwarded; arbitrary project or host secrets are not copied into the child.
