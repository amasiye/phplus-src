...
[Versioning](versioning.md). The current compiler version is `dev-2026.3.1`.

> **Status:** Stages 0–12, the post-Stage-12 semantic closure, Stages 13A–13D, and the post-Stage-13C completion gate are complete. The compiler includes deterministic recoverable production builds, certified mixed PHP/++PHP interoperability, stable catalog-owned diagnostics, structured generic context, process-free compiler-owned type-flow analysis, portable PHP/dependency declarations, and content-addressed incremental evidence. Stage 14 is next.
