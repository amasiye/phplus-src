...
> **Status:** Stages 0–12, the post-Stage-12 semantic closure, and Stages 13A–13B are complete. The compiler includes deterministic production builds, certified mixed PHP/++PHP interoperability, stable catalog-owned diagnostics, structured generic context, and process-free compiler-owned type-flow analysis. Stage 13C is next.
...
Project symbol tables record effective native/PHPDoc callable and property contracts in addition to classes, interfaces, traits, enums, constants, parameters, inheritance, namespaces, source files, and precise spans. `CallableContractResolver` is the single path for functions, methods, constructors, ordinary PHP, configured stubs, and reviewed intrinsics. Argument binding covers names, defaults, variadics, unpacking, and references; generic inference applies receiver and callable substitutions. One shared member resolver handles inheritance, interfaces, traits, unions, intersections, nullable/nullsafe receivers, visibility, static form, and asymmetric property access. The editor continues to resolve definitions and semantic tokens against the same symbol table.
...
The canonical `examples/mixed-application` project and `composer verify:mixed-application` exercise cross-language calls, generic and checked-error metadata, stub enrichment, autoload files, bootstrap relocation, manifest/source-map integrity, optimized Composer loading, and source-free execution. The maintained `tests/Fixtures/GenericContext/ShoppingCart` project exercises dependent generic bounds, generic property hooks and members, collection callbacks, foreach bindings, nested member access, lowering, linting, and execution. Stage 13A added the typed capability catalog, differential golden, and process-free compiler-only browser Check protocol; Stage 13B adds compiler-owned call, member, return, property, name, PHP/stub-boundary, and reviewed-intrinsic flow without changing the native full default. There is no entry-point model, dependency-driven tree-shaking, incremental compilation, watch mode, deployment bundling, compiler-only build, or native compilation. Stage 13C is the next boundary.
